"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SessionSerializer = void 0;
const common_1 = require("@nestjs/common");
const passport_1 = require("@nestjs/passport");
const director_service_1 = require("../director.service");
const director_model_copy_1 = require("../model/director.model copy");
const sequelize_1 = require("@nestjs/sequelize");
let SessionSerializer = class SessionSerializer extends passport_1.PassportSerializer {
    constructor(directorService, DirectorModel) {
        super();
        this.directorService = directorService;
        this.DirectorModel = DirectorModel;
    }
    serializeUser(user, done) {
        done(null, user);
    }
    async deserializeUser(payload, done) {
        const user = await this.DirectorModel.findOne({
            where: { email: payload.email },
        });
        return user ? done(null, user) : done(null, null);
    }
};
exports.SessionSerializer = SessionSerializer;
exports.SessionSerializer = SessionSerializer = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)('DIRECTOR_SERVICE')),
    __param(1, (0, sequelize_1.InjectModel)(director_model_copy_1.Director)),
    __metadata("design:paramtypes", [director_service_1.DirectorService, Object])
], SessionSerializer);
//# sourceMappingURL=Serializer.director.js.map